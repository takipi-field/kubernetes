...
For this example, we'll use hostPath to mount files from the host into the pod. See [Volume](https://kubernetes.io/docs/concepts/storage/volumes/) for more details and options.
...
```console
# ls /takipi/lib
libHSAgent.so            libPocoUtil.so.12  libZingAgent.so  sparkle.jar
